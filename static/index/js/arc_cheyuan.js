	  ...
	  $("#nr0641").html(isLongCar);
		...
		var createTime1 = datas.startTime;
          ...
          if(carFile3){$("#nr0634").attr("src",carFile3);}
            ...
            $('#arc_nav_a1').attr("href",'/cheyuan?'+'&startp='+startProvince+'&startc='+startCity+'&endp='+endProvince+'&endc='+endCity)
            $('#arc_nav_a2').attr("href",'/cheyuan?'+'&startp='+startProvince+'&startc='+startCity+'&starta='+startArea+'&endp='+endProvince+'&endc='+endCity+'&enda='+endArea)
  	...
  	"strartAddress":startCity
		...
		success:function(res){
		  ...
		  $('.tj_list'+i+'  #tj_shiming').css("display","none")}
				...
				$('#arc_bottom_more').attr("href","/cheyuan?start="+startCity+"&end="+endCity)
				}
				if($('.tj_list').length==1){
