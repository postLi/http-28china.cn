			...
			$("#nr076").css("text-decoration","none");	
				 ...
				 $("#js019").append(s1+s2+s3);	     
					...
					if (ptype==1){
						...
						if(zhjg0){var zhjg0=parseFloat(zhjg0).toFixed(0);}
						var zhjg=datas.rangePrices[j].discountPrice;
						if(zhjg){var zhjg=parseFloat(zhjg).toFixed(0);}
	...
	$.ajax(
		...
		success:function(res){
			...
			for(var i=0;i<datas.length;i++){
                ...
                if(lightPrice){var lightPrice=parseFloat(lightPrice).toFixed(0);}
                if(heavyPrice){var heavyPrice=parseFloat(heavyPrice).toFixed(0);}
				var id=datas[i].id;
