...
function process1(){
	...
	$.ajax(
		...
		success:function(res){
		   	$("#js019 .price_box_item2").not(":eq(0)").remove();
		   	$("#js018 .price_box_item1").not(":eq(0)").remove();
		   var datas = res.data;	
		   ...
		   if(rangeLogo3){$("#nr0717").attr("src",rangeLogo3);}
		   	...
		   	for(var j=0;j<plength;j++){
					...
					var qhjg0_1=parseFloat(qhjg0_1).toFixed(2);
					var qhjg_1=datas.rangePrices[j].discountPrice;
					if(qhjg_1){
					var qhjg_1=parseFloat(qhjg_1).toFixed(2);
					}
			...
			$("#nr076").css("text-decoration","line-through");
				 ...
				 $("#js019").append(s1+s2+s3);	     
					...
					if (ptype==1){
					    ...
					    var zhjg0_1=parseFloat(zhjg0_1).toFixed(2);
						var zhjg_1=datas.rangePrices[j].discountPrice;
						if(zhjg_1){
						var zhjg_1=parseFloat(zhjg_1).toFixed(2)}
				 ...
				 $("#nr0743").html(startVolume2+"-"+endVolume2+"公斤"); 
				 if(!endVolume2){
				 	 $("#nr0741").html(startVolume2_1+"公斤以上");
				 	 $("#nr0743").html(startVolume2+"公斤以上"); 
				 }
...
function process3(startCity,endCity){
	...
	$.ajax(
		...
		success:function(res){
		   ...
		   if(companyName.length>13){companyName=companyName.substring(0,13)+".."}
...
function process08(startProvince,startCity,endProvince,endCity,rangeIds){
	...
	$.ajax(
		...
		url:"/api/aflc-portal/portalt/aflcRecommendTransportRange/v1/findOtherRanges",
		dataType: "json",
		data:JSON.stringify(
			{
			currentPage:1,
			pageSize:8,
			vo:{"startProvince":startProvince,"startCity":startCity,"endProvince":endProvince,"endCity":endCity,"rangeIds":rangeIds
			}			//JSON.stringify({})
		}		
		),	
		success:function(res){
			...
			for(var i=0;i<datas.length;i++){
				...
				var publishName = datas[i].publishName;
				if(publishName&&publishName.length>14)
				{publishName=publishName.substring(0,14)+".."}
