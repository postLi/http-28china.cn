...
;(function $($) {
  ...
  var order = {
    ...
    checkForm: function() {
      ...
      if ($('.start').val()) {
        order.data.startProvince = $('.start')
          .val()
          .split(',')[0]
        order.data.startCity = $('.start')
          .val()
          .split(',')[1]
      } else {
        layer.msg('出发地不能为空')
      ...
      if ($('.end').val()) {
        order.data.endProvince = $('.end')
          .val()
          .split(',')[0]
        order.data.endCity = $('.end')
          .val()
          .split(',')[1]
      } else {
        layer.msg('到达地不能为空')
    ...
    promise: function() {
      ...
      $.ajax({
        ...
        url:
          '/api/aflc-uc/usercenter/aflcCarInfo/v1/add?access_token=' +
          access_token,
        async: false,
        dataType: 'json',
