...
loadCssCode(cssStr);
  ...
  var mapObj = {
    ...
    setData: function(pos, addr, obj) {
      ...
      this.noinfo = false
      // $(".afwl-map-info").find("span").eq(0).text(this.thename)
      $(".afwl-map-info").find("span").eq(0).text(this.thepos)
    },
    clear: function(){
      this.thepos = ''
      this.thename = ''
    ...
    submitForm: function(){
      ...
      if(notDetail){
        this.thename = clickMapData.province + clickMapData.city + clickMapData.district
      }
      ...
      if(this.thename){
        ...
        // 判断是否选择到市
        if(/(北京|重庆|天津|上海|香港|澳门)/.test(obj.province)===false && !obj.city && !obj.district){
          this.tipCity()
          return false
        }
    ...
    show: function(el){
      ...
      if (window.AMap) {
        ...
        _this.loadJs = true
        var cbfn = function(){
        	$.when(_this.load('map/citymap.js'),_this.load('https://webapi.amap.com/maps?v=1.4.8&key=e61aa7ddc6349acdb3b57c062080f730&plugin=AMap.Autocomplete,AMap.PlaceSearch,AMap.Geocoder')).done(function(){
	          //echarts
	          _this.loadJs = false
	          _this._init();
...
function selectCity() {
  mapObj.initMap({
      name: clickMapData.province + clickMapData.city + (clickMapData.district || ''),
      pos: ''
  })
}
function renderMap(map,data,stitle){
  ...
  var echartObj = {
    ...
    loadProvince: function(name){
      ...
      $.getJSON(mapObj.baseurl +'map/province/'+ provinces[name] +'.json', function(data){
      ...
      if(!this.inited){
        ...
        if( params.name in provinces ){
          ...
          if(  special.indexOf( params.seriesName ) >=0  ){
            ...
            //显示县级地图
            $.getJSON(mapObj.baseurl +'map/city/'+ cityMap[params.name] +'.json', function(data){
