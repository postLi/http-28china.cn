...
function process_01(vo1){
	...
	$.ajax(
		...
		url:"/api/aflc-portal/portalt/aflcRecommendTransportRange/v1/gatewayHomePageList",
		dataType: "json",
		data:JSON.stringify(
			{
			currentPage:1,
	...
	$.ajax(
		...
		success:function(res){
			...
			for(var i=0;i<datas.length;i++){
				...
				else if(strartAddress.indexOf('特别行政区')!=-1){strartAddress=strartAddress.substring(0,2)}
					else{
					var strartAddress = strartAddress.substring(3,10);}}
			    if(endAddress.length>3){
			    				if(endAddress.indexOf('特别行政区')!=-1){endAddress=endAddress.substring(0,2);}
			    				else if(endAddress.indexOf('自治区')!=-1){endAddress=endAddress.split('自治区')[1]}
			    				else {
...
function wlgs_tj(){
	...
	$.ajax(
		...
		success:function(res){
			 $("#wlgs_tj1 li").not(":eq(0)").remove();
			 $("#wlgs_tj2 li").not(":eq(0)").remove();
			if(!res.data || res.data==""){ 	
			 	return
			 }
			var datas = res.data.list;			
	...
	$.ajax(
		...
		success:function(res){
			...
			for(var i=0;i<datas.length;i++){
			    ...
			    if(belongCityName&&belongCityName.length>12){belongCityName=belongCityName.substring(0,12)+'..'}
