...
if (detail && longitude && latitude) {
  $('#wangdian_list').html('为您推荐附近3公里的网点')
}
$('#companyName').val(companyName)
$('#tjcx_00 .all').attr(
  'href',
  UrlUpdateParams(window.location.href, 'parkId', '')
)
$('#tjcx_01 .all').attr(
  'href',
  UrlUpdateParams(window.location.href, 'belongBrandCode', '')
)
$('#tjcx_02 .all').attr(
  'href',
  UrlUpdateParams(window.location.href, 'otherServiceCode', '')
)
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      for (var i = 0; i < datas.length; i++) {
        ...
        if (pointAddress.length > 16) {
          pointAddress = pointAddress.substring(0, 16) + '..'
        }
        var contactsName = datas[i].contactsName
        var contactsTel = datas[i].contactsTel
        var mobile = datas[i].mobile
