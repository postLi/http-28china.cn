...
function getpwd(access_token) {
  ...
  $.ajax({
    ...
    async: false,
    url:
      '/api/aflc-uc/usercenter/aflcLogisticsCompanyAccout/v1/getAccoutInfo?access_token=' +
      access_token,
...
;(function($) {
    ...
    if ('undefined' != typeof c) {
      ...
      if (document.cookie && '' != document.cookie) {
        ...
        for (b = 0; b < a.length; b++)
          if (
            ((e = jQuery.trim(a[b])), e.substring(0, d.length + 1) == d + '=')
          ) {
  ...
  var YZM_API = {
    ...
    vcode: function(width, height) {
      ...
      return (
        this.url +
        '/aflc-common/common/aflcMemberCenter/v1/getImageCode?imageWidth=' +
        w +
        '&imageHeight=' +
        h
      )
  ...
  var login_page = {
    ...
    init: function() {
      ...
      $('.login-yzm').on('click', function() {
        YZM_API.checkVcode($('#uservcode2').val()).always(function(res) {
          var status = res.status
          console.log('yzm_status:' + status)
          var check = login_page.validate(status)
          if (check.done) {
            login_page.login(
              _this.login_data.username,
              _this.login_data.yzm,
    ...
    validate: function(status) {
      ...
      if ($('#useryzm').val()) {
        ...
        checkinfo.done = false
        checkinfo.err += '<br>请输入您的短信验证码'
      }
      if ($('#useragree').is(':checked')) {
      } else {
        console.log($('#useragree').is(':checked'))
    ...
    login: function(username, code, usertype) {
      ...
      return YZM_API.login(username + '|' + usertype, code)
        .done(function(res) {
          if (res.access_token) {
