...
function cy1() {
  $.ajax({
    ...
    success: function(res) {
      ...
      var title =
        startCity + startArea + '&nbsp;&rarr;&nbsp;' + endCity + endArea
      ...
      if (usualPlace) {
        usualPlace = usualPlace.substring(0, 10)
      }
      var qq = datas.qq
      if (!qq) {
        $('#qq').css('display', 'none')
      ...
      if (carFile1 == 'vestCarSourceFile') {
        carFile1 = ''
      }
      var src1 = '/templets/default/images/pic/bg1.png'
      if (!carFile1) {
        $('#nr0631').attr('src', src1)
        $('#nr0632').attr('src', src1)
      }
      if (!carFile2) {
        $('#nr0633').css('display', 'none')
      }
      if (!carFile3) {
        $('#nr0634').css('display', 'none')
      }
      if (carFile1) {
        $('#nr0631').attr('src', carFile1)
        $('#nr0632').attr('src', carFile1)
      }
      if (carFile2) {
        $('#nr0633').attr('src', carFile2)
      }
      if (carFile3) {
        $('#nr0634').attr('src', carFile3)
      }

      if (datas.driverDesc == null) {
        var driverDesc = '暂未填写简介信息！'
      } else {
        var driverDesc = datas.driverDesc.substring(0, 400)
      }
      ...
      $('#arc_nav_a2').html(
        startCity + startArea + '到' + endCity + endArea + '车源信息'
      )
      $('#arc_nav_a1').attr(
        'href',
        '/cheyuan/detail' +
    ...
    success: function(res) {
      ...
      if ($('.tj_list').length >= 10) {
        ...
        $('#arc_bottom_more').attr(
          'href',
          '/cheyuan/detail?start=' + startCity + '&end=' + endCity
        )
      }
      if ($('.tj_list').length == 1) {
