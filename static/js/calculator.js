...
Calculator._volume_cal = function(obj) {
  ...
  var _unit_volume0 =
    parseFloat(_unit_length) * parseFloat(_unit_width) * parseFloat(_unit_higth)
...
function process07(transportRangeId, weight, volume) {
  ...
  $.ajax({
    ...
    url:
      '/api/aflc-portal/order/fclOrder/v1/getEstimateTotalPieces?transportRangeId=' +
      transportRangeId +
      '&weight=' +
      weight +
      '&volume=' +
    ...
    success: function(res) {
      var datas = res.data
      if (datas) {
        forecastPrice = datas.forecastPrice
      }
    },
    error: function(err) {
...
$('.item-form  input').keyup(function() {
  console.log('试算价格：')
  price()
})
//试算价格 E
