  ...
  CityPicker.prototype = {
    ...
    render: function() {
      var p = this.getPosition(),
        ...
        textspan =
          ...
          '</span>',
        dropdown =
          '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
          ...
          (this.includeDem('district')
            ? '<a data-count="district">区名</a>'
            : '') +
          (this.includeDem('county') ? '<a data-count="county">县名</a>' : '') +
          '<span class="city-select-reset">清除</span>' +
          '</div>' +
          ...
          (this.includeDem('district')
            ...
            : '') +
          (this.includeDem('county')
            ? '<div class="city-select county" data-count="county"></div>'
            : '') +
          '</div></div>'
    ...
    bind: function() {
      ...
      if (this.$district) {
        this.$district.on(
          EVENT_CHANGE,
          (this._changeDistrict = $.proxy(function() {
    ...
    getText: function() {
      var text = ''
      this.$dropdown.find('.city-select').each(function(i, e) {
    ...
    feedText: function() {
      ...
      if (text) {
        ...
        this.$textspan
          .find('>.title')
          .html(text)
          .show() //this.getText() 换成 text 2018.12.19
      } else {
        this.$textspan
          .find('>.placeholder')
    ...
    output: function(type) {
      ...
      code =
        type === PROVINCE
          ? 86
          : type === CITY
            ? this.$province && this.$province.find('.active').data('code')
            : type === DISTRICT
              ? this.$city && this.$city.find('.active').data('code')
              : type === COUNTY
                ? this.$district && this.$district.find('.active').data('code')
                : code
      districts = $.isNumeric(code) ? ChineseDistricts[code] : null
      if (code !== 86 && code !== undefined) {
        //接口 2018.12.19
        if (type === 'city') {
          $(this.$district).html('<dl class="clearfix"><dd></dd></dl>') //点击省名清空区名DOM 2018.12.19
        }
        $.ajax({
          type: 'get',
          ...
          async: false,
          url:
            '/api/aflccommonservice/common/aflcCommonPCA/v1/findAflcCommonPCAByCode?code=' +
            code,
