  ...
  var order = {
    ...
    initEvent: function() {
      ...
      $('.order-submit-btn').on('click', function() {
        ...
        if (res) {
          ...
          var fahuo_name = $('#fahuo_name').val()
          var fahuo_mobile = $('#fahuo_mobile').val()
          console.log(
            start_position + end_position + goodname + fahuo_name + fahuo_mobile
          )
          vo = new Object()
          vo.fromCity = start_position
          vo.toCity = end_position
          vo.cargoName = goodname
          vo.customer = fahuo_name
          vo.customerPhone = fahuo_mobile
          _this.sendm()
        }
      })
      ...
      $('.continue-create-order').on('click', function() {
        ...
        if (location.href.indexOf('/create/order') !== -1) {
    ...
    sendm: function() {
      $.ajax({
        ...
        url: '/sendm/vote/iemsCustomerController/sendCustomerSms.do',
        dataType: 'json',
        data: JSON.stringify({
          //JSON.stringify({})
          smsJsonStr: JSON.stringify(vo)
        }),
        success: function(res) {
          //			if(!res.smsJsonStr||res.smsJsonStr==""){
          //				return}
          console.log('发送成功！')
        },
        error: function(err) {
    ...
    setSuccessPage: function(_data) {
      ...
      $('.order-to span').text(
        data.aflcOrderAddressWebDtoList[1].contacts +
          ' ' +
      ...
      $('.order-lineinfo-from span').text(
        data.aflcOrderAddressWebDtoList[0].provinceCityArea +
          data.aflcOrderAddressWebDtoList[0].viaAddress
      )
      $('.order-lineinfo-to span').text(
        data.aflcOrderAddressWebDtoList[1].provinceCityArea +
          data.aflcOrderAddressWebDtoList[1].viaAddress
      )
    ...
    setAddrInfo: function(inx, thename, thepos, obj) {
      ...
      this.data.aflcOrderAddressWebDtoList[inx].viaAddressCoordinate =
        thepos || ''
    ...
    checkForm: function() {
      ...
      if ($('.lineinfo input[type="checkbox"]').length) {
        if (
          $('.lineinfo input[type="checkbox"]:checked').length === 0 &&
          this.pageinfo.isfree
        ) {
          layer.msg('请选择专线。')
          return false
        }
      ...
      data.aflcOrderAddressWebDtoList[1].contactsPhone = $(
        ...
        .eq(1)
        .val()
      data.aflcOrderAddressWebDtoList[1].isSave = $('.order-contact-to input')
        .eq(2)
        .prop('checked')
    ...
    setCompanyList: function(data) {
      ...
      $.each(data, function(inx, ele) {
        ...
        $.each(ele.rangePrices, function(inx, ele) {
          if (ele.type === '1') {
            str1 +=
              '<li><span class="jietiv">' +
              ele.startVolume +
              '公斤' +
              (ele.endVolume ? '~' + ele.endVolume + '公斤' : '以上') +
              '</span> <span class="jietih">' +
              ele.primeryPrice +
              '元/公斤</span> <span class="jietid">折后价' +
              ele.discountPrice +
              '元/公斤</span></li>'
          } else {
            str2 +=
              ...
              ele.startVolume +
              '立方' +
              (ele.endVolume ? '~' + ele.endVolume + '立方' : '以上') +
              '</span> <span class="jietih">' +
              ele.primeryPrice +
              '元/立方</span> <span class="jietid">折后价' +
              ele.discountPrice +
              '元/立方</span></li>'
          }
        })
        var str3 = tpl2
          .replace('{lowest}', ele.lowerPrice)
          .replace('{listr}', str1)
        var str4 = tpl2
          .replace('{lowest}', ele.lowerPrice)
          .replace('{listr}', str2)
        s += tpl
          .replace('{checked}', '')
          .replace(/\{name\}/g, ele.publishName)
          .replace('{ico}', '')
          .replace('{lowest}', ele.lowerPrice)
          .replace('{perweight}', _this.findLowestPrice(ele.rangePrices, '1'))
          .replace('{weightstr}', str3)
          .replace('{pervolume}', _this.findLowestPrice(ele.rangePrices, '0'))
          .replace('{volumestr}', str4)
          .replace('{transportAging}', ele.transportAging)
          .replace('{transportAgingUnit}', ele.transportAgingUnit)
          .replace('{departureHzData}', ele.departureHzData)
          .replace('{departureHzTime}', ele.departureHzTime)
          .replace('{wlid}', ele.companyId)
          .replace('{id}', ele.id)
      })
      $('.lineinfo').html(s)
    },
