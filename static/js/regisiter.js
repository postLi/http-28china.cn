;(function($){
  ...
  var reg_page = {
    ...
    showStep: function(index) {
      $(".step-next>div").removeClass("current").eq(index).addClass("current");
      $(".stepinfo").removeClass("current").eq(index).addClass("current");
      $(".step_info").attr("class","step_info step_info_ul_"+(index+1));
      $('.ff-title span').removeClass('current').eq(index).addClass('current');
    },
    checkValid: function(name, checkFn, defaultErrTxt){
      ...
      var defer = $.Deferred();
      checkFn(val, defer).then(function(){
        console.log(4)
        reg_page.reg_data[name] = val;
        par.addClass('success').find('.tip').html('');
      }, function(err2){
        console.log(1)
        par.addClass('error').find('.tip').html(err2 || defaultErrTxt);
        reg_page.reg_data[name] = '';
      });
      return defer.promise();
    },
    checkMobile: function(){
      return this.checkValid('mobile', function(value, p){
        if(validReg.MOBILE.test(value)){
          AFWL_API.checkMobileIsRegister({
            ...
            if(res.status === 100){
              console.log(2)
              // 已经注册
              p.reject('手机号已存在，请更换其他手机号或点击<a href="/member/resetpassword.php" target="_blank">找回密码</a>');
            } else {
              console.log(3)
              // 没有注册
              p.resolve();
    ...
    checkPassword: function(){
      return this.checkValid('password', function(value, p){
        if(value.length >= 6){
          // 纯数字或者纯字母或者相同的字符都判定为弱密码
          if(/\s/.test(value)){
            p.reject('不允许有空格！');
          }else if(AFLC_VALID.ONLY_NUMBER.test(value) || AFLC_VALID.ONLY_LETTER.test(value) || /^(.)\1*$/.test(value)){
            p.reject('您设置的密码过于简单，请重新设置密码！');
          } else {
            p.resolve();
          }
    ...
    checkPasswordSure: function(){
      return this.checkValid('passwordSure', function(value, p){
    ...
    checkContactName: function(){
      return this.checkValid('contactsName', function(value, p){
        if(value){
          p.resolve();
        } else{
          p.reject();
    ...
    checkCompany: function(){
      return this.checkValid('companyName', function(value, p){
        if(value){
    ...
    checkVcode: function(){
      return this.checkValid('imageCode', function(value, p){
        if(value){
          AFWL_API.checkVcode(value).done(function(res){
            if(res.status === 200){
              p.resolve();
            } else {
              p.reject();
    ...
    checkSmsCode: function(){
      return this.checkValid('smsCode', function(value, p){
        var mobile = $('[name="mobile"]').val();
        if(value&& validReg.MOBILE.test(mobile)){
          AFWL_API.checkMobileCode(mobile,value).done(function(res){
            if(res.status === 200){
              p.resolve();
            } else {
              p.reject();
    ...
    checkCity: function(){
      return this.checkValid('belongCityName', function(value, p){
        if(value){
          p.resolve();
        } else{
          p.reject();
    ...
    doRegisiter: function(){
      AFWL_API.regisiterByMobile(this.reg_data).done(function(res){
        if(res.status === 200){
          reg_page.showStep(2);
          // 判断注册的是什么角色，显示对应的文案
          $('.reg_qiyewz').hide();
    ...
    init: function(){
      ...
      $('.getcode-btn').on('click',function(){
        ...
        if(_this.hasClass('disabled')){
          ...
          if(validReg.MOBILE.test(mobile)){
            ...
            AFWL_API.getMobileCode(mobile).done(function(res){
      ...
      $('.registiter-page .step-next2 .next-btn').on('click',function () {
        reg_page.validate().done(function(res){
      ...
      $('.registiter-page .step-next3 .last-btn,.tipinfo a').on('click',function (e) {
        e.preventDefault();
        

        $('[name="gourl"]').val('http://192.168.1.157:9526/baseInfo/authentication');
        login_page.login(reg_page.reg_data.mobile,reg_page.reg_data.password,reg_page.memberType, AFWL_API.constant.LINK_MEMBER + '/baseInfo/authentication');
        return false
      });
      ...
      $('.registiter-page .step-next3 .normal-btn').on('click',function (e) {
        ...
        login_page.login(reg_page.reg_data.mobile,reg_page.reg_data.password,reg_page.memberType);
      });
    }
  };
  var login_page = {
    ...
    init: function(){
      ...
      $(".login-btn").on("click", function(){
        ...
        if(check.done){
          ...
          if(_this.loginType === 'password'){
            ...
            login_page.loginByMobile(_this.login_data.username, _this.login_data.pwd, _this.login_data.usertype);
          }
        } else {
      ...
      window.addEventListener('message', function(e){
        ...
        if(payload.indexOf('code:')!==-1){
          var code = payload.split(':')[1];
    ...
    checkVcode: function(cb){
      ...
      if(vcode){
        AFWL_API.checkVcode(vcode).done(function(res){
          if(res.status === 200){
            cb && cb();
          } else {
            layer.alert('验证码有误');
    ...
    bindWx: function(username,openid,type,unionid){
      return AFWL_API.bindWxUser(username,openid,type,'',unionid).done(function(res){
        if(res.status === 200){
          // 绑定成功
          layer.msg("绑定成功。");
          // location.href='/';
    ...
    openBindPop: function(){
      layer.open({
        ...
        title: '绑定微信号', //不显示标题
        content: '<div class="login-box2"> <h4>您的微信号未绑定，请输入手机号进行绑定</h4> <div class="inputbox"> <span class="icon icon-mobile"></span> <input type="text" maxlength="11" name="userid2" id="usermobile2" placeholder="请输入您的手机号"> </div> <div class="inputbox smscodeinput"> <span class="icon icon-password"></span> <input type="text" maxlength="10" name="smscode2" id="usersmscode2" placeholder="请输入短信验证码"> <span class="getsmscode2">获取验证码</span> </div> <div class="login-btn2">绑 定</div> </div>'
     ...
     return AFWL_API.getWxInfo().done(function(res){
        ...
        seajs.use('wxLogin', function(){
          var obj = new WxLogin({
            ...
            appid: data.appid, 
            scope: data.scope, 
            redirect_uri: encodeURIComponent("http://www.28china.cn/plus/list.php?tid=78"),
            state: data.state,
            style: "",
            href: ""
          });
    ...
    loginPhpByToken: function(access_token, usertype, url){
      return AFWL_API.loginByToken(access_token).done(function(res){
        if(res.data){
          return AFWL_API.phplogin(res.data.userName, res.data.memberPwd, true).done(function(){
    ...
    if(domain.indexOf('28china.cn')!==-1){
      domain = '.28china.cn';
          ...
          $.cookie('access_token', res.access_token,{ expires: 7,domain: domain, path: '/' });
          $.cookie('login_mobile', username,{ expires: 7,domain: domain, path: '/' });
          $.cookie('login_type', usertype,{ expires: 7,domain: domain, path: '/' });
          $.cookie('loginId', res.data.id,{ expires: 7, path: '/' });
          $.cookie('loginCompanyName', res.data.companyName,{ expires: 7, path: '/' });
          // $('#form1').submit();
          AFWL_API.phplogin(username, pwd).done(function(){
            // 登录成功
            location.href = url || '/';
          }).fail(function(res){
            layer.alert('登录失败：'+(res.errorInfo || res.message || res.text || JSON.stringify(res) || '未知错误'));
...
//mobile=2&imageCode=3&smsCode=4&password=5&passwordSure=6
  var find_page = {
    ...
    checkValid: function(name, checkFn, errTxt){
      ...
      par.removeClass('success error');
      var defer = $.Deferred();
      checkFn(val, defer, el).then(function(){
        find_page.find_data[name] = val;
        par.addClass('success').find('.tip').html('');
      }, function(){
        par.addClass('error').find('.tip').html(errTxt);
        find_page.find_data[name] = '';
      });
      return defer.promise();
    },
    validate: function(){
      var mobile = this.checkValid('mobile', function(value, p){
        if(validReg.MOBILE.test(value)){
          p.resolve();
        } else{
          p.reject();
      ...
      var password = this.checkValid('password', function(value, p){
        if(value.length >= 6){
          p.resolve();
        } else{
          p.reject();
      ...
      var passwordSure = this.checkValid('passwordSure', function(value, p){
        if(value && value === $('[name="password"]').val()){
          p.resolve();
        } else{
          p.reject();
      ...
      var contactsName = this.checkValid('contactsName', function(value, p){
        if(value){
          p.resolve();
        } else{
          p.reject();
      ...
      var companyName = this.checkValid('companyName', function(value, p){
        if(value){
      ...
      var imageCode = this.checkValid('imageCode', function(value, p,el){
        if(value){
          AFWL_API.checkVcode(value).done(function(res){
            if(res.status === 200){
              p.resolve();
      ...
      var smsCode = this.checkValid('smsCode', function(value, p){
        var mobile = $('[name="mobile"]').val();
        if(value&& validReg.MOBILE.test(mobile)){
          AFWL_API.checkMobileCode(mobile,value).done(function(res){
            if(res.status === 200){
      ...
      find_page.find_data.belongCityName = $('#store-selector .text div').attr('title');
      find_page.find_data.belongCity = $('#store-selector .text div').attr('data-id');
      find_page.find_data.provinceCode = $('#store-selector .text div').attr('data-province');
      find_page.find_data.cityCode = $('#store-selector .text div').attr('data-city');
      find_page.find_data.areaCode = $('#store-selector .text div').attr('data-area');

      return $.when(mobile,password,passwordSure,contactsName,companyName,imageCode,smsCode,defer.promise());
    },
    doRegisiter: function(){
      AFWL_API.regisiter(this.find_data).done(function(res){
        if(res.status === 200){
    ...
    init: function(){
      ...
      $('.password-page .step_2 .next-btn').on('click',function () {
        ...
        AFWL_API.checkMobileCode(find_page.find_data.mobile,value).done(function(res){
          if(res.status === 200){
            find_page.showStep(2);
          } else {
            layer.alert('验证失败');
      ...
      $('.password-page .step_3 .next-btn').on('click',function (e) {
        ...
        AFWL_API.findPassword(find_page.find_data).done(function(res){
          if(res.status === 200){
            layer.alert('修改成功');
      ...
      $('.password-page .step-next3 .normal-btn').on('click',function () {
        $('[name="gourl"]').val('/index.php');
        login_page.login(find_page.find_data.mobile,find_page.find_data.password,find_page.memberType);
      });
    }
  };
...
$.each(cityData, function(el, index){
  cityLiTag += '<li><a href="#none" data-value="{code}">{name}</a></li>'.replace('{code}', index.code).replace('{name}', index.name);
});
...
var provinceHtml = '<div class="content"><div data-widget="tabs" class="m JD-stock" id="JD-stock">' +
  ...
  '<div class="mc" data-area="0" data-widget="tab-content" id="stock_province_item">' + '    <ul class="area-list">' +
  cityLiTag +
  '    </ul>' + '</div>' + '<div class="mc" data-area="1" data-widget="tab-content" id="stock_city_item"></div>' +
  '<div class="mc" data-area="2" data-widget="tab-content" id="stock_area_item"></div>' +
  '<div class="mc" data-area="3" data-widget="tab-content" id="stock_town_item"></div>' + '</div></div>';
  ...
  if (result && result.length > 0) {
      for (var i = 0, j = result.length; i < j; i++) {
          ...
          if (result[i].name.length > 12) {
              longerhtml.push("<li class='longer-area'><a href='#none' data-value='" + result[i].code + "'>" + result[i]
                  .name + "</a></li>");
          } else if (result[i].name.length > 5) {
              longhtml.push("<li class='long-area'><a href='#none' data-value='" + result[i].code + "'>" + result[i].name +
                  "</a></li>");
          } else {
              html.push("<li><a href='#none' data-value='" + result[i].code + "'>" + result[i].name + "</a></li>");
          }
      }
  } else {
...
function getStockOpt(id, name) {
  ...
  var address = $.trim(currentAreaInfo.currentProvinceName + currentAreaInfo.currentCityName + currentAreaInfo.currentAreaName +
      currentAreaInfo.currentTownName);
  var address2 = $.trim(currentAreaInfo.currentProvinceName + cleanKuohao(currentAreaInfo.currentCityName) +
  cleanKuohao(currentAreaInfo.currentAreaName) + cleanKuohao(currentAreaInfo.currentTownName));
  var repeatREG = /^(.*)\1\1$/;
  address = address.replace(repeatREG, '$1');
  address2 = address2.replace(repeatREG, '$1');
  $("#store-selector .text div").html(address2).attr("title",
      address).attr('data-id', id).attr('data-province',currentAreaInfo.currentProvinceId).attr('data-city',currentAreaInfo.currentCityId).attr('data-area',currentAreaInfo.currentAreaId);
}
  ...
  if (currentAreaInfo.currentLevel >= 2) {
      ...
      if (page_load) {
          ...
          if (currentAreaInfo.currentAreaId && new Number(currentAreaInfo.currentAreaId) > 0) {
              getStockOpt(currentAreaInfo.currentAreaId, currentDom.find("a[data-value='" + currentAreaInfo.currentAreaId +
                  "']").html());
          } else {
    ...
    cityContainer.html(getAreaList(provinceCityJson["" + provinceId]));
      ...
      if (page_load) {
          if (currentAreaInfo.currentCityId && new Number(currentAreaInfo.currentCityId) > 0) {
              chooseCity(currentAreaInfo.currentCityId, cityContainer.find("a[data-value='" + currentAreaInfo.currentCityId +
                  "']").html());
          } else {
