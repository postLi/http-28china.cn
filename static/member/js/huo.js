  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      for (var i = 0; i < datas.length; i++) {
        ...
        var startProvinceCityArea = datas[i].startProvinceCityArea.substring(
          3,
          9
        )
        ...
        if (goodsNum.length > 4) {
          goodsNum = goodsNum.substring(0, 4) + '..'
        }
        if (goodsWeight.length > 4) {
          goodsWeight = goodsWeight.substring(0, 4) + '..'
        }
        if (goodsVolume.length > 4) {
          goodsVolume = goodsVolume.substring(0, 4) + '..'
        }
