...
//专线搜索 S
	...
	$('#wlLineTo .select-item').each(function (i,e) {
      ...
      endc=encodeURI(endc)
      enda=encodeURI(enda)
     window.location=uid+'-line.html?startp='+startp+'&startc='+startc+'&starta='+starta+'&endp='+endp+'&endc='+endc+'&enda='+enda;
    })


    $("#search_wlLine2").click(
...
var totalPage=1;
	$.ajax(
        ...
        if(start&&start.length>6){start=start.substring(0,6)+".."}
				...
				var orderurl="/member/"+uid+".html?action=order"+"&id="+id+"&publishId="+publishId;
