...
//var rangeIds="";
	$.ajax(
		...
		success:function(res){
		  ...
		  if(recommendType==1){$('.tj_list'+i+' #tj_icon_2').css("display","inline");}
...
var totalPage=8;
	$.ajax(
		...
		url:"/api/aflc-portal/portalt/aflcRecommendTransportRange/v1/listRanges",
		dataType: "json",
		async:false,
		data:JSON.stringify(
			{
       ...
       if(discountPrice1){zhjg=discountPrice1}
		    ...
		    var collateral=datas[i].collateral;
				...
				if(!datas[i].rangeLogo){var rangeLogo1=""}
				if(datas[i].rangeLogo){var rangeLogo1=datas[i].rangeLogo;
				if(rangeLogo1.indexOf(",")){var rangeLogo1=rangeLogo1.split(",")[0]; }
				}
        ...
        if(!rangeLogo1){ $(".nr_a21_img img").attr("src",src1);}
        // if(!rangeLogo1){ $("#nr01").attr("src",src1);}
				if(rangeLogo1){
        $(".nr_a21_img img").attr("src",rangeLogo1);
  	...
  	$('.wlzx_list'+i+' #list_shiming').css("display","none")}
	  	...
	  	if(!qq){
	  		...
	  		if(recommendType==1){$('.wlzx_list'+i+' #tj_icon_1').css("display","inline");}
